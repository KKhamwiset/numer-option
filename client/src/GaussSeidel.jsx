...
const GuassSeidel = () => {
  ...
  const solve = () => {
      ...
      while (error > epsilon && iteration < maxIterations) {
          ...
          error = calculateError(xPrevious, x);
          iteration++;
          steps.push({
              explanation: `Iteration ${iteration}:`,
              latex: `x^{(${iteration})} = \\begin{bmatrix} ${x.map(val => val.toFixed(6)).join(' & ')} \\end{bmatrix} \\\\ \\text{Error} = ${error.toFixed(6)}`
          });
      }
  ...
  return (
    <div className="flex flex-col items-center mt-20">
      <h2 className="text-center text-5xl mb-10">Guass Seidel Method</h2>
